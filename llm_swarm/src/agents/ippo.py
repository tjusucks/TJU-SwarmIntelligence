        ...
        if is_independent:
            ...
            for k in state_dict:
                if k.startswith("models."):
                    ...
                    break
            if first_agent_prefix is not None:
                new_state_dict = {}
                prefix = f"models.{first_agent_prefix}."
...
class IndependentPolicies(nn.Module):
    """Wrapper for independent actor-critic networks per agent."""
    ...
    def forward(self, obs: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        for i, agent_id in enumerate(self.agent_ids):
            ...
            mean, value = model.forward(obs[i : i + 1])
    ...
    def get_dist_and_value(
        ...
        for i, agent_id in enumerate(self.agent_ids):
            model = self.models[agent_id]
            mean, value = model.forward(obs[i : i + 1])
            log_std = torch.clamp(model.log_std, model.log_std_min, model.log_std_max)
            std = torch.exp(log_std).expand_as(mean)
            means.append(mean.squeeze(0))
            stds.append(std.squeeze(0))
            values.append(value.squeeze(0))
        dist = Normal(torch.stack(means), torch.stack(stds))
        return dist, torch.stack(values)
    ...
    def load_state_dict(self, state_dict: dict[str, torch.Tensor], strict: bool = True) -> tuple[list[str], list[str]]:
        ...
        if is_shared:
            ...
            for agent_id in self.agent_ids:
                for k, v in state_dict.items():
                    new_state_dict[f"models.{agent_id}.{k}"] = v.clone()
            return super().load_state_dict(new_state_dict, strict=strict)
        else:
            return super().load_state_dict(state_dict, strict=strict)
...
class IPPOTrainer:
    """Independent PPO trainer supporting both shared policy and independent policies."""
    ...
    def __init__(
        ...
        if self.share_policy:
            ...
            if agent_ids is None:
                raise ValueError("agent_ids must be provided when share_policy=False")
            self.model = IndependentPolicies(
                agent_ids=agent_ids,
                obs_dim=obs_dim,
                action_dim=action_dim,
                hidden_size=config.hidden_size,
        ...
        # Safe optimizer load_state_dict wrapping
        original_load = self.optimizer.load_state_dict
        def safe_load_state_dict(state_dict):
            try:
                original_load(state_dict)
            except Exception as e:
                print(f"Warning: Could not load optimizer state dict: {e}. Resetting optimizer states.")
        self.optimizer.load_state_dict = safe_load_state_dict
    ...
    def act(
        ...
        if self.share_policy:
            ...
            with torch.no_grad():
                for i, agent_id in enumerate(self.agent_ids):
                    model = self.model.models[agent_id]
                    dist, value = model.get_dist_and_value(obs_t[i : i + 1])
                    sampled_action = dist.sample()
                    action = torch.clamp(sampled_action, -1.0, 1.0)
                    logprob = dist.log_prob(action).sum(dim=-1)

                    actions_list.append(action.cpu().squeeze(0))
                    logprobs_list.append(logprob.cpu().squeeze(0))
                    values_list.append(value.cpu().squeeze(0))

            return (
                torch.stack(actions_list).numpy(),
                torch.stack(logprobs_list).numpy(),
                torch.stack(values_list).numpy(),
            )
    ...
    def value(self, obs: np.ndarray) -> np.ndarray:
        ...
        if self.share_policy:
            ...
            with torch.no_grad():
                for i, agent_id in enumerate(self.agent_ids):
                    model = self.model.models[agent_id]
                    _, value = model.get_dist_and_value(obs_t[i : i + 1])
                    values_list.append(value.cpu().squeeze(0))
            return torch.stack(values_list).numpy()
        ...
        if self.share_policy:
            ...
            for _ in range(self.cfg.update_epochs):
                ...
                for start in range(0, batch_size, self.cfg.minibatch_size):
                    ...
                    mb_obs = torch.as_tensor(b_obs[mb], dtype=torch.float32, device=self.device)
                    mb_obs = torch.nan_to_num(mb_obs, nan=0.0, posinf=self.cfg.obs_clip, neginf=-self.cfg.obs_clip)
                    mb_obs = torch.clamp(mb_obs, -self.cfg.obs_clip, self.cfg.obs_clip)
                    mb_actions = torch.as_tensor(b_actions[mb], dtype=torch.float32, device=self.device)
                    mb_old_logprobs = torch.as_tensor(b_old_logprobs[mb], dtype=torch.float32, device=self.device)
                    mb_adv = torch.as_tensor(b_advantages[mb], dtype=torch.float32, device=self.device)
                    mb_returns = torch.as_tensor(b_returns[mb], dtype=torch.float32, device=self.device)
                    mb_old_values = torch.as_tensor(b_values[mb], dtype=torch.float32, device=self.device)

                    dist, new_values = self.model.get_dist_and_value(mb_obs)
                    new_logprobs = dist.log_prob(mb_actions).sum(dim=-1)
                    entropy = dist.entropy().sum(dim=-1).mean()

                    log_ratio = torch.clamp(new_logprobs - mb_old_logprobs, -self.cfg.max_log_ratio, self.cfg.max_log_ratio)
                    ratio = torch.exp(log_ratio)
                    pg_loss_1 = -mb_adv * ratio
                    pg_loss_2 = -mb_adv * torch.clamp(ratio, 1.0 - self.cfg.clip_coef, 1.0 + self.cfg.clip_coef)
                    actor_loss = torch.max(pg_loss_1, pg_loss_2).mean()

                    v_clipped = mb_old_values + torch.clamp(new_values - mb_old_values, -self.cfg.clip_coef, self.cfg.clip_coef)
            ...
            # Loop over agents and update each policy with its specific data
            for i, agent_id in enumerate(self.agent_ids):
                model = self.model.models[agent_id]
                a_obs = obs[:, i, :]
                a_actions = actions[:, i, :]
                a_old_logprobs = old_logprobs[:, i]
                a_adv = advantages[:, i]
                a_ret = returns[:, i]
                a_val = values[:, i]

                # Normalize advantages for this agent
                adv_mean = float(np.mean(a_adv))
                adv_std = float(np.std(a_adv))
                if adv_std < 1e-8:
                    adv_std = 1.0
                a_adv = (a_adv - adv_mean) / adv_std
                a_adv = np.nan_to_num(a_adv, nan=0.0, posinf=10.0, neginf=-10.0)

                batch_size = a_obs.shape[0]
                idxs = np.arange(batch_size)
                ...
                for _ in range(self.cfg.update_epochs):
                    ...
                    for start in range(0, batch_size, self.cfg.minibatch_size):
                        ...
                        mb_obs = torch.as_tensor(a_obs[mb], dtype=torch.float32, device=self.device)
                        mb_obs = torch.nan_to_num(mb_obs, nan=0.0, posinf=self.cfg.obs_clip, neginf=-self.cfg.obs_clip)
                        mb_obs = torch.clamp(mb_obs, -self.cfg.obs_clip, self.cfg.obs_clip)
                        mb_actions = torch.as_tensor(a_actions[mb], dtype=torch.float32, device=self.device)
                        mb_old_logprobs = torch.as_tensor(a_old_logprobs[mb], dtype=torch.float32, device=self.device)
                        mb_adv = torch.as_tensor(a_adv[mb], dtype=torch.float32, device=self.device)
                        mb_returns = torch.as_tensor(a_ret[mb], dtype=torch.float32, device=self.device)
                        mb_old_values = torch.as_tensor(a_val[mb], dtype=torch.float32, device=self.device)

                        dist, new_values = model.get_dist_and_value(mb_obs)
                        new_logprobs = dist.log_prob(mb_actions).sum(dim=-1)
                        entropy = dist.entropy().sum(dim=-1).mean()
            ...
            self.buffer.clear()
            updates = max(1, total_updates)
            return {
                "actor_loss": actor_loss_sum / updates,
                "critic_loss": critic_loss_sum / updates,
